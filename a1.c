...
void moveCloud() {
    /*Move the cloud*/
    static int resetTime = 1;
    //static time_t start, end; // return current time in seconds
    int diff;
    ...
    if (resetTime == 1) {
        ...
        /*Determine if three seconds have passed*/
        if (diff >= 35) {
            //printf("3 second has passed\n");
            updateCloudLoc();
            resetTime = 1;
        }
    }
    ...
    for (y = cloudLine; y < WORLDY-1; y++) {
        for (z = WORLDZ - 1; z >= 0; z--) {
            for (x = WORLDX - 1; x >= 0; x--) {
                printf("location in array %d, %d, %d value=%d\n", x, y, z, world[x][y][z]);

                if (world[x][y][z] == 5) {
                    printf("Found a cloud at %d, %d, %d \n", x, y, z);
                    /*Remove old cloud*/
                    world[x][y][z] = 0;
